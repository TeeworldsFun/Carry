...
void CCharacter::HandleNinja()
{
	...
	if ((Server()->Tick() - m_Ninja.m_ActivationTick) > (g_pData->m_Weapons.m_Ninja.m_Duration * Server()->TickSpeed() / 1000) && g_Config.m_SvInsta == 0)
	{
		// time's up, return
		...
		// check if we Hit anything along the way
		{
			...
			for (int i = 0; i < Num; ++i)
			{
				...
				if(g_Config.m_SvInsta == 0)
					...
					aEnts[i]->Die(m_pPlayer->GetCID(), WEAPON_NINJA);
			}
		}
...
void CCharacter::HandleWeapons()
{
	...
	if(AmmoRegenTime && g_Config.m_SvInsta == 0)
	{
